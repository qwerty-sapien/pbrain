...
"""Launch the sibling swarm UX harness against this pbrain checkout.

The actual adaptive harness lives beside this repo at ``../swarm``. This script
keeps the pbrain-side invocation deterministic: it passes the repo's
...
def _swarm_env() -> dict[str, str]:
    ...
    env.setdefault("PBRAIN_AUTO_YES", "1")
    return env


def _quote_env(env: dict[str, str]) -> str:
    keys = ["PYTHONPATH", "PBRAIN_AUTO_YES"]
