...
DEFAULT_PRODUCT_NAME = "pbrain"
DEFAULT_QUARANTINE_FOLDER = "Learning/Inbox/pb"
DEFAULT_DATA_HOME = "~/.local/share/pbrain"
DEFAULT_STATE_HOME = "~/.local/state/pbrain"
...
def normalize_provider_name(provider: str) -> str:
    """Return the canonical provider key used in config and model bindings."""
...
def _parse_role_binding(binding: str) -> tuple[str, str]:
    ...
    return normalize_provider_name(provider), model.strip()
...
class Config(BaseModel):
    ...
    def _normalize(self) -> "Config":
        ...
        self.llm.provider = normalize_provider_name(self.llm.provider)
        if self.providers:
            normalized_providers: dict[str, ProviderConfig] = {}
            for name, provider in self.providers.items():
                normalized_providers[normalize_provider_name(name)] = provider
            self.providers = normalized_providers

        if not self.providers:
            provider_name = normalize_provider_name(self.llm.provider)
        ...
        if not self.model_roles.default:
            provider_name = normalize_provider_name(self.llm.provider or next(iter(self.providers)))
...
def get_config_path(path: Optional[Path] = None) -> Path:
    ...
    env_override = os.environ.get("PBRAIN_CONFIG_PATH")
    if env_override:
        return Path(os.path.expanduser(env_override))

    xdg_config = os.environ.get("XDG_CONFIG_HOME", os.path.expanduser("~/.config"))
    return Path(xdg_config) / "pbrain" / "config.toml"
...
def _propagate_default_model(payload: dict[str, Any], new_model: str) -> None:
    ...
    provider = normalize_provider_name(payload.get("llm", {}).get("provider", "gemini"))
...
def set_default_model_binding(binding: str, *, path: Optional[Path] = None) -> Config:
    """Persist the preferred default binding while preserving generated fast tiers."""
    cfg = get_config(path, force_reload=True)
