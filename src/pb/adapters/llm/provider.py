...
"""LLM provider interface backed by the configured provider runtime."""
...
from pb.storage.config import normalize_provider_name
...
class ConfiguredLLMProvider(LLMProvider):
    """Provider adapter backed by the configured pbrain LLM runtime."""
    ...
    def __init__(self, model: str | None = None, provider_name: str | None = None):
        ...
        if provider_name:
            ...
            if model and ":" not in model:
                ...
                provider_cfg = self._runtime.config.providers.get(provider)
                default_model = provider_cfg.default_model if provider_cfg else ""
                if not default_model:
                    raise RuntimeError(f"No default model is configured for provider {provider}.")
                binding = f"{provider}:{default_model}"
        self.provider, self.model = self._runtime._resolve_provider_and_model(binding)
        self._client = self._runtime._client_for_provider(self.provider)
        if not self._client.is_available():
            raise RuntimeError(
                "pb requires an LLM for this workflow.\n\n"
                "Run:\n"
                "  pb init llm\n\n"
                "Then configure credentials for your selected provider."
            )
    ...
    def _generate(self, prompt: str, *, max_output_tokens: int = 800) -> str:
        ...
        if result.error is not None:
            raise RuntimeError(result.error.raw_message)
...
def get_provider(provider_name: Optional[str] = None) -> LLMProvider:
    ...
    logger.debug("llm.get_provider", requested=provider_name or "default")
    return ConfiguredLLMProvider(provider_name=provider_name)
