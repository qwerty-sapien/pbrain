...
from pb.storage.config import Config, ProviderConfig, get_config, normalize_provider_name
...
class LLMRuntime:
    ...
    def _resolve_provider_and_model(self, model: Optional[str] = None) -> tuple[str, str]:
        ...
        if ":" not in selected:
            ...
            if default_provider == "gemini":
                return default_provider, resolve_model(selected, fallback=FLASH_MODEL)
            return default_provider, selected
        provider, resolved_model = _parse_role_binding(selected)
        return provider, resolved_model

    def health(self, provider_name: str | None = None, model_name: str | None = None) -> LLMHealth:
        ...
        if not model_name:
            ...
            model_name = provider_cfg.default_model or self.config.llm.default_model
        ...
        if provider_name == "gemini":
            message = (
                ...
                if available
                else "Gemini support is not ready. Install the `gemini` extra and set GEMINI_API_KEY or GOOGLE_CLOUD_PROJECT."
            )
    ...
    def live_probe(self, *, model: Optional[str] = None, timeout: int = 12) -> LLMProbeResult:
        ...
        health = self.health(provider_name, selected_model)
        client = self._client_for_provider(provider_name)
    ...
    def require(self, purpose: str) -> LLMHealth:
        ...
        if not health.available:
            raise RuntimeError(
                "pbrain requires an LLM for this workflow.\n\n"
    ...
    def make_stage_recorder(self, workflow: str, intent: str, *, route_hint: str = ""):
        ...
        try:
            ...
            try:
                ...
                data_dir = Path(tempfile.gettempdir()) / "pbrain"
