...
Default log location (when enabled):
    macOS:   ~/Library/Logs/pbrain/events.jsonl
    Linux:   $XDG_STATE_HOME/pbrain/events.jsonl
             (fallback: ~/.local/state/pbrain/events.jsonl)
    Other:   ~/.pbrain/logs/events.jsonl
"""
...
def _default_log_path() -> Path:
    ...
    if system == "Darwin":
        return Path.home() / "Library" / "Logs" / "pbrain" / "events.jsonl"
    if system == "Linux":
        state_home = os.environ.get("XDG_STATE_HOME", "")
        if not state_home:
            state_home = str(Path.home() / ".local" / "state")
        return Path(state_home) / "pbrain" / "events.jsonl"
    return Path.home() / ".pbrain" / "logs" / "events.jsonl"
