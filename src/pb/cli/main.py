...
# Canonical pbrain command surface
...
app.add_typer(init_mod.app, name="init", rich_help_panel="Setup",
              help="Set up pbrain for the first time")
...
def update_pb(
    ...
    """Update this pbrain checkout with a fast-forward pull."""
...
def reset_pb(
    ...
    """Delete vault contents and reset pbrain SQLite state."""
    runtime = ctx.obj["runtime"]
...
def _handle_no_config_degradation(ctx: typer.Context) -> None:
    ...
    console = get_console()
    console.print()
    console.print("[bold]Welcome to pbrain[/bold] -- a terminal-native study and practice system.")
    console.print()
...
def main(
    ...
    """pbrain - terminal-native study and deliberate-practice system."""
    ...
    if config_path is not None:
        os.environ["PBRAIN_CONFIG_PATH"] = str(config_path)
    if yes:
        os.environ["PBRAIN_AUTO_YES"] = "1"
