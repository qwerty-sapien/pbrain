...
_CLIENT_CONFIG_PATHS = {
    ...
    "generic": Path.home() / ".config" / "pbrain" / "mcp-example.json",
}
...
def mcp_print_config(
    ...
    config_path: str = typer.Option("", "--config", help="Optional explicit pbrain config path."),
) -> None:
...
def mcp_status() -> None:
    ...
    server_path = shutil.which("pb-mcp")
...
def mcp_pending() -> None:
    ...
    import pb.mcp.tools.pbrain  # noqa: F401
