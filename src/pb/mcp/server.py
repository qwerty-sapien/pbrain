...
"""FastMCP server for pbrain integrations.
...
logger = logging.getLogger("pb-mcp")
mcp = FastMCP("pb-mcp")


def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="pb-mcp", add_help=True)
    parser.add_argument("--vault", default=None, help="Vault profile name to serve")
    parser.add_argument("--config", default=None, help="Explicit pbrain config.toml path")
...
def main() -> None:
    ...
    args = _parse_args()
    if args.config:
        os.environ["PBRAIN_CONFIG_PATH"] = args.config
    ...
    try:
        ...
        from pb.mcp.tools import pbrain  # noqa: F401
