...
pbrain works best when you give it a learning target, let it shape one block, then close the loop with a finish note.

## Install And Initialize

For normal CLI use from any folder:
...
Use `uv add probrain` only inside an existing Python project when you want
pbrain as a dependency. It needs a `pyproject.toml`, so it will fail in a normal
folder that is not a Python project.

If the package is not published in your index yet, install the CLI from the
canonical source:
