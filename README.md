...
pbrain is a terminal-based system for turning learning intent into the next concrete session: study, deliberate practice, evidence, and recall.
...
- Exposes the `pb-mcp` server for agent clients while keeping the CLI as the primary interface.

## Install

Requires Python 3.11+.

For normal CLI use from any folder, install `pb` as a tool:

```bash
uv tool install probrain
pb init
pb doctor
```

`uv add probrain` is only for adding pbrain as a dependency to an existing
Python project with a `pyproject.toml`; it is not the normal CLI install path.
If the package is not published in your index yet, install from the canonical
source instead:
...
- `pb`, `pbrain`, `brain`: open the same interactive learning shell.
...
LLM-backed flows use configured `provider:model` bindings. Gemini, OpenAI,
OpenRouter, and Claude through Anthropic share the same runtime path; read-only
and deterministic paths still work without a live model.

Gemini remains the default provider. The locked Gemini model IDs are:

- `gemini-3.1-flash-lite-preview`
- `gemini-3-flash-preview`
- `gemini-3.1-pro-preview`

Use `pb model use openai:gpt-5-mini`, `pb model use claude:claude-sonnet-4-5`,
or `pb model use gemini:gemini-3-flash-preview` to switch the default binding.
Install optional extras only for integrations you use, such as `gemini`, `anki`,
`pdf`, `embeddings`, `batch`, `google`, `kaggle`, `scraping`, or `watch`.

Use `pb doctor` for local setup and `pb doctor --llm` only when you want a live provider probe.
